...
管理者画面は同じアプリ内の `?admin=1` 付きURLで開きます。通常画面からはリンクしません。
`STUDY_ROOM_ADMIN_PASSWORD` を必ず設定してください。
...
5. Secretsで `STUDY_ROOM_ADMIN_PASSWORD` を設定する
6. 管理者画面は `https://あなたのアプリURL/?admin=1` から開く
...
Streamlit Community Cloudでは、通常の自習室画面と同じアプリ内の隠しURLから管理者画面を開きます。
...
Streamlit Community Cloudで使う場合は、アプリのSecretsに以下を設定します。
